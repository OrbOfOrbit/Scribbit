...
class editViewController: UITableViewController  {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 0{
            ...
            alert.addAction(UIAlertAction(title: "finished", style: .default, handler: { [weak alert] (_) in
                let textField = alert!.textFields![0]
                self.basiclist.items[indexPath.row-1].content = textField.text!
                self.basiclist.items[indexPath.row-1].done=true
            }))
            alert.addAction(UIAlertAction(title: "unfinished", style: .default, handler: { [weak alert] (_) in
                let textField = alert!.textFields![0]
                self.basiclist.items[indexPath.row-1].content = textField.text!
